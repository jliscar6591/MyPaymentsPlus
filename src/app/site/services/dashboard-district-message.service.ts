...
export class DashboardDistrictMessageService {
	...
	public getContent(loginResponse: LoginResponseModel): Observable<number> {
		...
		let failureMessage: string = 'Failed to get district message.';

        let Url = Constants.WebApiUrl.Auth + '/DistrictMessage' ;  // URL to web API
		...
		this.http.get(Url, options)
			...
			data => {
				...
				this.districtKey = data.json().districtKey;				
			},
			error => {
	...
	public getName(loginResponse: LoginResponseModel): Observable<number> {
		let newJwt: any;
		let token = loginResponse.access_token;
		this.loginResponse = loginResponse;
